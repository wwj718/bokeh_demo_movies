...
movies = psql.read_sql(query, conn)  #12569 rows × 26 columns , try  it in jupyter
...
min_year = Slider(title=u"Year released", start=1940, end=2014, value=1970, step=1)
max_year = Slider(title=u"End Year released", start=1940, end=2014, value=2014, step=1)
...
genre = Select(title=u"体裁", value="All",
               options=open(join(dirname(__file__), 'genres.txt')).read().split())
director = TextInput(title=u"导演名字模糊搜索")
cast = TextInput(title=u"演员名字模糊搜索")
x_axis = Select(title="X 轴", options=sorted(axis_map.keys()), value="Tomato Meter")
y_axis = Select(title="Y 轴", options=sorted(axis_map.keys()), value="Number of Reviews")
